...
// We create instruction schema and serialize function inside of our class.
export class Movie {
  title: string;
  rating: number;
  description: string;
  ...
  // We send our instruction to the prgrm in a serialized format. wow! that'd b fast.
  borshInstructionSchema = borsh.struct([
    borsh.u8("variant"), // uint(8bit) represents which instruction should be executed (in other words which function on the program should be called). // !? I dont get it. Does it represent the instruction or the program?
    borsh.str("title"),
    borsh.u8("rating"),
    borsh.str("description"),
  ]);

  serialize(): Buffer { // serialize function returns us a buffer containing instruction schema.
    const buffer = Buffer.alloc(1000)
    this.borshInstructionSchema.encode({ ...this, variant: 0 }, buffer) // ...this >>> { title, rating, description, variant }
